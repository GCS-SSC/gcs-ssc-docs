...
Agreement GET/PATCH requires both extension switches and exact agreement read/update access. PATCH locks authorization and the extension lifecycle, reauthorizes, re-resolves current config, validates `{ tags }`, and upserts extension KV. Proponent GET requires exact applicant-recipient/Team read access and removes stored tags whose source is no longer visible. Proponent writes—and field-keyed agreement writes—are persisted by the corresponding core profile-update hooks inside the host transaction; suggestions only update the form payload and are not saved until the surrounding profile save succeeds.
