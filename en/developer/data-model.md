...
Review and recommendation integrity uses subtype-check triggers for assessment/checklist schema and runtime rows, immutable published-version triggers, and `trg_fn_reset_additional_reviewer_completion` when an additional-reviewer assignment changes. `trg_fn_enforce_completion_audit_fields` keeps completion value, actor, and timestamp coherent. Workflows persist ordered setup members, typed nested default-owner mappings, runtime items, immutable status transitions, and owner blockers. `trg_fn_validate_workflow_setup_member` enforces one reference matching the member kind and parent target; `trg_fn_validate_workflow_member_owner` enforces the matching nested identity and active user. `trg_fn_lock_workflow_status_transition` makes transition history immutable. Every named function is paired with the same-suffix trigger listed in the ledger.
...
Agreement, amendment, claim, claim reconciliation, forecast, commitment, payment, monitor, and Closeout rows receive registered `Common_Entity` identities. Their assignment triggers enforce/retire exact rosters. `Funding_Case_Agreement_Closeout` permits one open row per Agreement, constrains status/open combinations, and owns an immutable readiness snapshot per run. Snapshot triggers validate the `close_out` run and parent Agreement and reject update/delete. `uuid_generate_v7` supplies revision identifiers where native support is unavailable. Initial working-version triggers create budget/activity versions. Approval-submission triggers validate run purpose/target and reject packet update/delete. Revision validation requires the submission, optional amendment, and Agreement to agree.
