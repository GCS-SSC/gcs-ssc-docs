...
Les routes GET/PATCH d'entente exigent les deux commutateurs. GET utilise `agreement:read` de portée; PATCH exige le plafond Contributeur ou supérieur et l’affectation exacte de l’entente, puis verrouille l'autorisation et le cycle de vie, autorise de nouveau, résout la configuration courante, valide `{ tags }`, puis insère ou met à jour le stockage clé-valeur. La route GET du demandeur-bénéficiaire utilise `applicant_recipient:read` de portée et retire les étiquettes dont la source n'est plus visible. Ses écritures, ainsi que les écritures d'entente par champ, sont persistées par les crochets de mise à jour des profils de base dans la transaction hôte déjà autorisée; les suggestions modifient seulement la charge du formulaire jusqu'à l'enregistrement réussi du profil.
