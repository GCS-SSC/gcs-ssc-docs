...
L’ordre doit être un entier positif inutilisé par un membre actif. Le modèle d’approbation, s’il est fourni, doit être un modèle valide appartenant à ce volet. Un échec ne crée ni membre partiel ni schéma orphelin.

Utilisez plutôt **Associer un schéma** lorsque le schéma de l’agence existe déjà.

## Publication du schéma

Enregistrer valide le schéma de travail. Publier autorise de nouveau l’opération du volet, crée une ligne de version immuable, marque sa publication comme publiée et avance sa version de publication immuable.
