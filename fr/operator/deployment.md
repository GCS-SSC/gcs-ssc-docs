...
L’application fournit `bun run railway:demo:reset` pour son environnement GCS Demo précisément fixé. Par défaut (ou avec `--preview`), la commande affiche la portée prévue sans réinitialisation. `--execute` est destructif et doit être lancé par l’exploitant dans son vrai terminal interactif, sans redirection d’entrée ou de sortie. Le script refuse la suppression de fichiers par un agent et vérifie projet, environnement, service, volume et mode de démonstration; ce n’est pas une commande générale de reprise en production.

La réinitialisation utilise le dernier `main` capturé au début, un déploiement de maintenance, la base PostgreSQL dédiée et le répertoire `/files` du volume de démonstration. Elle restaure douze modèles de démonstration et vérifie le déploiement obtenu. Le stockage d’objets externe et l’ancien répertoire PGlite sont exclus. Suspendez les publications GitHub et déploiements concurrents; un déploiement qui remplace celui du script arrête l’opération.

Les services de base endormis sont réveillés et vérifiés avant toute destruction. Les échecs SSH produisent des diagnostics ciblés pour les clés, la vérification d’hôte, l’authentification ou les cibles indisponibles. Après un échec partiel, le script restaure sa source locale de déploiement sans redéployer automatiquement par-dessus la réinitialisation incomplète. Examinez la phase et l’environnement avant reprise. N’utilisez jamais cette commande à la place des sauvegardes de production décrites plus haut.
